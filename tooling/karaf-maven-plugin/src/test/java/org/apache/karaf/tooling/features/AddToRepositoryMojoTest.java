...
public class AddToRepositoryMojoTest {
    ...
    public void testRegex() throws Exception {
        ...
        Feature tempFeature = new Feature();
        testFeature.setName("temp");
        testFeature.setVersion("1.0.0");

        Map<String, Feature> featuresMap = new HashMap<>();
        featuresMap.put("test/1.0.0", testFeature);
        featuresMap.put("temp/1.0.0", tempFeature);
        ...
        Assert.assertEquals(2, features.size());
        Iterator<Feature> iterator = features.iterator();
        Feature checkTest = iterator.next();
        Assert.assertEquals("test", checkTest.getName());
        Assert.assertEquals("1.0.0", checkTest.getVersion());

        Feature checkTemp = iterator.next();
        Assert.assertEquals("temp", checkTemp.getName());
        Assert.assertEquals("1.0.0", checkTemp.getVersion());
    }
